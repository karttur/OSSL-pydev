...
def ScatterCorrectDF(trainDF, testDF, scattercorrection, columns):
    ...
    return scattCorrTxt, trainDF, testDF, M1
                      
def SpectraScatterCorrectionFromDf(trainDF, testDF, scattercorrection):
    """ Scatter correction for spectral signals
    ...
    scattCorrTxt, trainDF, testDF, scatCorrMeanSpectraL = ScatterCorrectDF(trainDF, testDF, scattercorrection, columns)
    
    #key = next(iter(trainD))
...
class RegressionModels:
    ...
    def _Filtering(self):
        ...
        columnsX = [item for item in self.spectraDF.columns]
        #columnsX = [item for item in self.X_train]
       
        # extract the covariate columns as X
        X = self.spectraDF[columnsX]
        
        # If the filter is a kernel (including moving average)    
        if self.filtering.movingaverage.kernel:
 
            sumkernel =  np.asarray(self.filtering.movingaverage.kernel).sum()
            
            normkernel = self.filtering.movingaverage.kernel/sumkernel
            
            X1 = convolve1d(X, normkernel, axis=-1, mode=self.filtering.movingaverage.mode)
            
            #Xtest = convolve1d(self.X_test, normkernel, axis=-1, mode=self.filtering.movingaverage.mode)
            
            filtertxt = 'kernel filter'
                 
        # If Gaussian filter
        elif self.filtering.Gauss.sigma:
                        
            X1 = gaussian_filter1d(X, self.filtering.Gauss.sigma, axis=-1, mode=self.filtering.Gauss.mode)
                
            #Xtest = gaussian_filter1d(self.X_test, self.filtering.Gauss.sigma, axis=-1, mode=self.filtering.Gauss.mode)
                
            filtertxt = 'Gaussian filter'
             
        # If Savitsky Golay filter
        elif self.filtering.SavitzkyGolay.window_length:
            
            X1 = savgol_filter(X, window_length=self.filtering.SavitzkyGolay.window_length, 
                               polyorder= self.filtering.SavitzkyGolay.polyorder,
                               axis=-1,
                               mode=self.filtering.SavitzkyGolay.mode)
            
            #Xtest = savgol_filter(self.X_test, window_length=self.filtering.SavitzkyGolay.window_length, 
            #                   polyorder= self.filtering.SavitzkyGolay.polyorder,
            #                   axis=-1,
            #                   mode=self.filtering.SavitzkyGolay.mode)
    ...
    def _MultiFiltering(self):
        ...
        for r, rang in enumerate(self.multifiltering.beginWaveLength):
            ...
            self.filtering.movingaverage.kernel = []
            self.filtering.Gauss.sigma = 0
            self.filtering.SavitzkyGolay.window_length = 0
                        
            if self.multifiltering.movingaverage.kernel[r]:
                
                self.filtering.movingaverage.kernel = self.multifiltering.movingaverage.kernel[r]
                
            elif self.multifiltering.SavitzkyGolay.window_length[r]:
                                
                self.filtering.SavitzkyGolay.window_length = self.multifiltering.SavitzkyGolay.window_length[r]
                
            elif self.multifiltering.Gauss.sigma[r]:
                                
                self.filtering.Gauss.sigma = self.multifiltering.Gauss.sigma[r]
                
            # Run the filtering
            self._Filtering()
            
            # Half the output band width to adjust the input bands to read
            halfwlstep = self.multifiltering.outputBandWidth[r]/2
                
            if self.multifiltering.mode == 'noendpoints':
                
                # Define the output wavelengths
                outputWls = np.arange(self.multifiltering.beginWaveLength[r]+self.multifiltering.outputBandWidth[r], self.multifiltering.endWaveLength[r]-1, self.multifiltering.outputBandWidth[r])
            
            else:
                
                # Define the output wavelengths
                outputWls = np.arange(self.multifiltering.beginWaveLength[r], self.multifiltering.endWaveLength[r]+1, self.multifiltering.outputBandWidth[r])
            ...
            for row in self.spectraDF.values:
                ...
                spectraA = np.interp(outputWls, wlArr, row,
                            left=self.multifiltering.beginWaveLength[r]-halfwlstep,
                            right=self.multifiltering.endWaveLength[r]+halfwlstep)
    ...
    def _SplitDataSet(self):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = model_selection.train_test_split(self.Xall, self.y, test_size=self.modelTests.trainTest.testSize)
    ...
    def _RegrModTrainTest(self, multCompAxs):
        ...
        r2_total = r2_score(self.y_test, predict)
        
        rmse_total = sqrt(mean_squared_error(self.y_test, predict))
        
        medae_total = median_absolute_error(self.y_test, predict)
        
        mae_total = mean_absolute_error(self.y_test, predict)
        
        mape_total = mean_absolute_percentage_error(self.y_test, predict)
        ...
        if self.modelTests.trainTest.plot:
            ...
            title = ('Model: %(mod)s; RMSE: %(rmse)2f; r2: %(r2)2f' \
                      % {'mod':name,'rmse':mean_squared_error(self.y_test, predict),'r2': r2_score(self.y_test, predict)} )

            txtstr = ('RMSE: %(rmse)2f\nr2: %(r2)2f\nnTrain: %(i)d\nnTest: %(j)d' \
                      % {'rmse':self.trainTestResultD[self.targetFeature][name]['rmse'],
                         'r2': self.trainTestResultD[self.targetFeature][name]['r2'],
                         'i': self.X_train.shape[0], 'j': self.X_test.shape[0]})

            self._PlotRegr(self.y_test, predict, suptitle, title, txtstr, '',name, 'trainTest', multCompAxs)
    ...
    def _FeatureImportance(self, multCompAxs):
        ...
        #columns = [item for item in self.spectraDF.columns]
        ...
        maxFeatures = min(self.featureImportance.reportMaxFeatures, len(columns))

        # Permutation importance
        n_repeats = self.featureImportance.permutationRepeats

        permImportance = permutation_importance(model, self.X_test, self.y_test, n_repeats=n_repeats)
    ...
    def _UnivariateSelector(self):
        ...
        self.targetFeatureSelectedD[self.targetFeature]['nFeaturesRemoved'] = nfeatures-X_train.shape[1]

        self.targetFeatureSelectionTxt = ' - %s %s' %( nfeatures-X_train.shape[1] ,'SelKBest')
    ...
    def _PermutationSelector(self):
        ...
        n_features_to_select = self.modelFeatureSelection.permutationSelector.n_features_to_select
        ...
        permImportance = permutation_importance(model, self.X_test, self.y_test)
        ...
        self.modelFeatureSelectedD[self.targetFeature][name]['nFeaturesRemoved'] = nfeatures - self.X_train.shape[1]

        self.modelFeatureSelectionTxt = '- %s %s' %( nfeatures - self.X_train.shape[1], 'PermutSel')
    ...
    def _RFESelector(self):
        ...
        self.modelFeatureSelectionTxt = '- %s %s' %(len(discardL),'RFE')
        ...
        self.X_train = pd.DataFrame(self.X_train, columns=selectL)
        
        self.X_test = pd.DataFrame(self.X_test, columns=selectL)

        #self.modelSelectD[name] = selectL

    def _StandardisationOLD(self):
        """
        """
        IGNORESPREPROCESS
        scalertxt = 'None'
    ...
    def _PcaPreprocess(self):
        ...
        if (len(columns) < self.pcaPreproc.n_components):
    ...
    def _WardClustering(self, n_clusters):
        ...
        agglomeratetxt = '%s input features clustered to %s covariates using  %s' %(nfeatures,n_clusters,self.agglomeratedFeaturesD['method'])
...
class MachineLearningModel(Obj, RegressionModels):
        ...
        if self.plot.rows.apply:
            ...
            if self.plot.rows.targetFeatures.apply:
                ...
                for regrModel in self.regressorModels:
                    ...
                    suptitle += "%s; %s; %s; %s; \n" %(self.scatterCorrectiontxt, self.scalertxt, self.pcaPreproctxt, self.hyperParamtxt)
    ...
    def _PilotModeling(self,rootFP,sourcedatafolder,dstRootFP, multCompFig, multCompAxs):
        ...
        # Set the subplot
        # MOVED
        self.scatterCorrectiontxt, self.scalertxt, self.pcaPreproctxt, self.hyperParamtxt = 'NA','NA','NA','NA'
        ...
        if self.filtering.apply:
                
            filtertxt = self._Filtering()
                
        elif self.multifiltering.apply:
        ...
        for self.targetN, self.targetFeature in enumerate(self.targetFeatures):
            ...
            self.scatterCorrectiontxt = 'scatter correction: None'
                      
            if self.scattercorrection.apply:
           
                scatterCorrectiontxt, self.X_train, self.X_test, self.scattCcorrMeanSpectra = SpectraScatterCorrectionFromDf(self.X_train, self.X_test, self.scattercorrection)
                
                self.scatterCorrectiontxt = 'scatter correction: %s' %(scatterCorrectiontxt)
            ...
            self.scalertxt = 'scaling: %s' %(scaler)
            ...
            # PCA preprocess    
            self.pcaPreproctxt = 'pca: None'
                    
            if self.pcaPreproc.apply:
                
                self._PcaPreprocess()
                
                self.pcaPreproctxt = 'pca: %s comps' %(self.pcaPreproc.n_components)
...
def SetupProcesses(iniParams):
    ...
    modelPlotLayoutD = ReadAnyJson(iniParams['modelplotlayout'])
    ...
    for jsonObj in jsonProcessObjectL:
        ...
        paramD['plot'] = modelPlotLayoutD['modelPlotLayout']
